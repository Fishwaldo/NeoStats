...
static int ms_hail( CmdParams* cmdparams )
{
	...
	irc_prefmsg( ms_bot, target, 
		...
		cmdparams->av[0], cmdparams->av[0], cmdparams->av[0] );
	return NS_SUCCESS;     
}

/** @brief ms_lapdance
 *
 *  lapdance command handler
 *
 *  @cmdparams pointer to commands param struct
 *
 *  @return NS_SUCCESS if suceeds else NS_FAILURE
...
static int ms_ode( CmdParams* cmdparams )
{
	...
	irc_prefmsg( ms_bot, cmdparams->source, "Ode to %s sent to %s!", 
		cmdparams->av[0], target->name );
	irc_prefmsg( ms_bot, target, "Courtesy of your friend %s:", 
		cmdparams->source->name );
	irc_prefmsg( ms_bot, target, "*recites*" );
	irc_prefmsg( ms_bot, target, "How I wish to be a %s,", cmdparams->av[0] );
	irc_prefmsg( ms_bot, target, "a %s I would like to be.", cmdparams->av[0] );
	irc_prefmsg( ms_bot, target, "For if I was a %s,", cmdparams->av[0] );
...
static int ms_poem( CmdParams* cmdparams )
{
	...
	irc_prefmsg( ms_bot, cmdparams->source, 
		"Poem about %s sent to %s!", cmdparams->av[0], target->name );
	irc_prefmsg( ms_bot, target, "Courtesy of your friend %s:", cmdparams->source->name );
	irc_prefmsg( ms_bot, target, "*recites*" );
	irc_prefmsg( ms_bot, target, "I wish I was a %s,", cmdparams->av[0] );
	irc_prefmsg( ms_bot, target, "A %s is never glum,", cmdparams->av[0] );
