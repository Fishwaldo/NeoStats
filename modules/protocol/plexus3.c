...
** $Id: plexus.c 3021 2006-01-26 15:30:22Z Fish $
*/
...
{
	send_cmd( "%s %s 1 %lu %s %s %s %s %s %s%lu :%s", MSG_NICK, nick, ts, newmode, ident, host, server, nick, host, ts, realname );
}
...
{
	if( !srv ) {
		do_nick( argv[0], argv[1], argv[2], argv[4], argv[5], argv[6], NULL, NULL, argv[3], argv[8], argv[9], NULL, NULL );
	} else {
