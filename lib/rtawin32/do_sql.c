  ...
  for (rx = 0; rx < nr; rx++)
  {
    ...
    for (wx = 0; wx < cmd.nwhrcols; wx++)
    {
      ...
      if (cmd.ptbl->tabletype == TBL_LIST)
      {
        ...
        {
          pd = (int)lnode_get(node) + cmd.pwhr[wx]->offset;
        }
      }
      else if (cmd.ptbl->tabletype == TBL_HASH)
      {
        ...
        {
          pd = (int)hnode_get(hnode) + cmd.pwhr[wx]->offset;
        }
      }
      else
      {
      ...
      *buf++ = 'D';             /* Data packet */
      ...
      for (cx = 0; cx < cmd.ncols; cx++)
      {
        ...
        if (cmd.ptbl->tabletype == TBL_LIST)
        {
          ...
          {
            pd = (int)lnode_get(node) + cmd.pcol[cx]->offset;
          }
        }
        else if (cmd.ptbl->tabletype == TBL_HASH)
        {
          ...
          {
            pd = (int)hnode_get(hnode) + cmd.pcol[cx]->offset;
          }
        }
        else
        {
