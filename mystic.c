...
aCtab cFlagTab[] = {
		...
		{MODE_STIRPBADWORDS,	'U', 0, 0}, /* no badwords */
		{0x0, 0x0, 0x0, 0x0}
};
...
int ssmo_cmd(const char *from, const char *umodetarget, const char *msg) {
	notice(s_Services, "Warning, Module %s tried to SMO, which is not supported in MysticIRCd", segvinmodule);
	nlog(LOG_NOTICE, LOG_CORE, "Warning, Module %s tried to SMO, which is not supported in MysticIRCd", segvinmodule);
	return 1;
}
...
int sswhois_cmd(const char *target, const char *swhois) {
	notice(s_Services, "Warning Module %s tried to SWHOIS, which is not supported in MysticIRCd", segvinmodule);
	nlog(LOG_NOTICE, LOG_CORE, "Warning. Module %s tried to SWHOIS, which is not supported in MysticIRCd", segvinmodule);
	return 1;
}
