...
** $Id: dotconf.c,v 1.8 2003/05/26 09:18:28 fishwaldo Exp $
*/
			...
			*cp2 = '\0';
			...
			if (opt.name[0] == 0)
			...
			else if (opt.type == ARG_STR)
				{
					...
					/* skip whitespace */ 
					while ( (cp3 < eob) && (*cp3 != '\0') && (isspace(*cp3)) )
						cp3++;
			...
			eos = cp2 + CFG_MAX_VALUE - 1;
			sq = 0; dq = 0;                   /* clear quoting flags */
			while ( (*cp1 != '\0') && (cp2 != eos) && (word_count < CFG_VALUES) )
				{
					switch(*cp1)
						{
						case '\'':               /* single quote */
							if (dq) break;         /* already double quoting, break out */
							if (sq) { sq--; }      /* already single quoting, clear state */
							else if (!sq) { sq++; }/* set state for single quoting */
							break;
						case '"':                /* double quote */
							if (sq) break;         /* already single quoting, break out */
							if (dq) { dq--; }      /* already double quoting, clear state */
							else if (!dq) { dq++; }/* set state for double quoting */
							break;
						default: break;
						}
